...
public class Event_DojoAgent
{
    ...
    public static boolean warpStartAgent(final MapleCharacter c, final boolean party) {
        final int stage = 1;
        final int mapid = 970030100;
        final ChannelServer ch = c.getClient().getChannelServer();
        for (int i = 970030100; i < 970030110; ++i) {
    ...
    public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting) {
        try {
                ...
                if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
                    for (final MaplePartyCharacter mem : c.getParty().getMembers()) {
                        ...
                        if (chr != null) {
                            ...
                            int cspoints = (int)(((double)thisStage * 3.3 + (double)(thisStage - pLevel) * 4.0) * 1.1);
                            if (cspoints <= 0) {
                                cspoints = 5;
                            }
                    ...
                    int cspoints2 = (int)(((double)thisStage * 3.3 + (double)(thisStage - c.getLevel()) * 5.0) * 1.3);
                    if (cspoints2 <= 0) {
                        cspoints2 = 5;
                    }
                    c.modifyCSPoints(2, 5, true);
                    c.setDojo(c.getDojo() + point2);
                    c.getClient().sendPacket(MaplePacketCreator.Mulung_Pts(point2, c.getDojo()));
                }
            }
            ...
            FilePrinter.printError("武陵道場.txt", (Throwable)rm);
        }
        return false;
    }
    ...
    private static final void spawnMonster(final MapleMap map, final int stage) {
        ...
        if (mobid != 0) {
        	final int mobid2 = mobid;
            final int rand = Randomizer.nextInt(3);
            MapTimer.getInstance().schedule((Runnable)new Runnable() {
                @Override
                public void run() {
                    map.spawnMonsterWithEffect(MapleLifeFactory.getMonster(mobid2), 15, (rand == 0) ? point1 : ((rand == 1) ? point2 : point3));
                }
            }, 3000L);
        }
    }
