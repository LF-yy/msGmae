...
public class MapleFootholdTree
{
    ...
    public MapleFootholdTree(final Point p1, final Point p2) {
        ...
        this.sw = null;
        this.se = null;
        this.footholds = new LinkedList<MapleFoothold>();
        this.depth = 0;
        this.p1 = p1;
        this.p2 = p2;
    ...
    private final List<MapleFoothold> getRelevants(final Point p) {
        return this.getRelevants(p, (List<MapleFoothold>)new LinkedList<MapleFoothold>());
    }
    
    private final List<MapleFoothold> getRelevants(final Point p, final List<MapleFoothold> list) {
        list.addAll((Collection<? extends MapleFoothold>)this.footholds);
        if (this.nw != null) {
        ...
        for (final MapleFoothold fh2 : this.footholds) {
            if (fh2.getX1() <= tox && fh2.getX2() >= tox && fh2.getY1() <= toy && fh2.getY2() >= toy) {
                if (fhdata.getId() != fh2.getId() && fh2.getId() != fhdata.getNext() && fh2.getId() != fhdata.getPrev()) {
                    System.err.println("Couldn't find the correct pos for next/prev");
                    return false;
                }
                return true;
            }
    ...
    public final MapleFoothold findBelow(final Point p) {
        ...
        for (final MapleFoothold fh : xMatches) {
            if (!fh.isWall() && fh.getY1() != fh.getY2()) {
                ...
                final double s3 = (double)Math.abs(p.x - fh.getX1());
