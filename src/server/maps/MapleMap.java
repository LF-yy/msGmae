...
public class MapleMap {
    ...
    private boolean monsterCanMove = false;
...
//          List<MonsterDropEntry> dropEntry = chr.retrieveDrop(mob.getId());
...
//        Collections.shuffle(dropEntry);
     ...
     double itemDropm =   ((chServerrate * chr.getDropMod()
                * (showdown / 100)
                * lastDrop / 100.0 * chr.getDropRateChr()
                + (chr.getItemDropm() / 100.00) + (chr.getDropm()>1 ? (chr.getDropm()-1) : 0)));

        HideAttribute hideAttribute = Start.hideAttributeMap.get(chr.getId());
        if(hideAttribute.totalDropRate>0){
            itemDropm += (double) (hideAttribute.totalDropRate / 100.0);
        }
        for (MonsterDropEntry de2 : chr.retrieveDrop(mob.getId())) {
            ...
            if (de2.itemId == 0) {
                //金币掉落
                int mesos = Randomizer.nextInt(1 + Math.abs(de2.Maximum+hideAttribute.totalMesoRateCount))+de2.Minimum ;
                if(mesos>de2.Maximum){
                    mesos = de2.Minimum;
                }
                if (mesos > 0) {
                    ...
                    if (chr.isDropOnMyBag() && !mob.getStats().isFfaLoot() && !chr.isDropItemFilter(0)) {
                        int meso = (int) ((double) mesos * chr.getExpRateChr() * (lastMeso / 100.0) * ((double) (chr.getVipExpRate() / 100) + 1.0) * (double) chr.getDropMod() * (double) chr.getMesoRateChr() * (double) cmServerrate+chr.getDropm()*(LtMS.ConfigValuesMap.get("砍爆率") / 100.0));
                        ...
                        this.spawnMobMesoDrop((int) ((double) mesos * (lastMeso / 100.0) * ((double) (chr.getVipExpRate() / 100) + 1.0) * (double) chr.getDropMod() * (double) chr.getMesoRateChr() * chr.getDropm() * (double) cmServerrate*(LtMS.ConfigValuesMap.get("砍爆率") / 100.0)), chr.getPosition(), mob, chr, false, droptype);
                    } else {
                        this.spawnMobMesoDrop((int) ((double) mesos * (lastMeso / 100.0) * ((double) (chr.getVipExpRate() / 100) + 1.0) * (double) chr.getDropMod() * (double) chr.getMesoRateChr() * chr.getDropm() * (double) cmServerrate*(LtMS.ConfigValuesMap.get("砍爆率") / 100.0)), this.calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
                    }
                }
            } else {
                int i = Randomizer.nextInt(de2.Maximum + hideAttribute.totalDropRateCount - de2.Minimum <= 0 ? 1 : de2.Maximum + hideAttribute.totalDropRateCount +1);

                if (extracted(de2.itemId, chr, Math.max(i, 1))) {
                    ...
                    if (GameConstants.getInventoryType(de2.itemId) == MapleInventoryType.EQUIP) {
                        ...
                        idrop = new Item(de2.itemId, (short) 0, (short)Math.max(i, 1), (byte) 0);
                    }
        ...
        //全局掉落
        for ( MonsterGlobalDropEntry de3 : mi.getGlobalDrop()) {
    ...
    private void dropFromMonsterTwo(MapleCharacter chr, final MapleMonster mob) {
        ...
        int i1 = Randomizer.nextInt(9999999);
        int ran = 1000;
        if(i1>0 && Objects.nonNull(Start.ltMonsterValueMap)){
            //现金实充额度标准  只有实充超过这个数值才不会进入低概率
            if (chr.getDonate() < (Objects.nonNull(LtMS.ConfigValuesMap.get("现金实充额度标准")) ? LtMS.ConfigValuesMap.get("现金实充额度标准") : 500)){
                ran =  Randomizer.nextInt(1000);
            }
            if (ran>998) {
                Map<String, Integer> mobInfo = Start.ltMonsterValueMap.get(mob.getId());
                if (Objects.nonNull(mobInfo) && mobInfo.get("probability") > i1 && mobInfo.get("exposure_value") >0 && mobInfo.get("number") >0) {
                    //提现额度增加
                    int flags = Start.setltMonstervalue(mobInfo.get("monster_id"), mobInfo.get("exposure_value"));
                    if (flags==1) {
                        Start.setltMonstervalueRecord(chr.getId(), mobInfo.get("monster_id"), mob.getName(), mobInfo.get("exposure_value"));
                        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[现金额度] : 恭喜 [" + chr.getName() + "] 成为天选之人,对" + mob.getName() + "上下其手成功从其身上摸到了现金额度[" + mobInfo.get("exposure_value") + "点],大家快来羡慕他(她)吧!!!=>个人信息可查看额度！"));
                        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[现金额度] : 恭喜 [" + chr.getName() + "] 成为天选之人,对" + mob.getName() + "上下其手成功从其身上摸到了现金额度[" + mobInfo.get("exposure_value") + "点],大家快来羡慕他(她)吧!!!=>个人信息可查看额度！"));
                        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[现金额度] : 恭喜 [" + chr.getName() + "] 成为天选之人,对" + mob.getName() + "上下其手成功从其身上摸到了现金额度[" + mobInfo.get("exposure_value") + "点],大家快来羡慕他(她)吧!!!=>个人信息可查看额度！"));
                        Bot.sendGroupMessage("[现金额度] : 恭喜 [" + chr.getName() + "] 成为天选之人,在怪物公园,对" + mob.getName() + "上下其手成功从其身上摸到了现金额度[" + mobInfo.get("exposure_value") + "点],大家快来羡慕他(她)吧!!!");
                    }
                }
            }
        }
        for (MonsterDropEntry de2 : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) >= ((de2.itemId == 1012168 || de2.itemId == 1012169 || de2.itemId == 1012170 || de2.itemId == 1012171) ? de2.chance : drop * (LtMS.ConfigValuesMap.get("砍爆率") / 100.0))) {
                continue;
            }
            if (mesoDropped && droptype != 3 && de2.itemId == 0) {
                continue;
            ...
            if (de2.itemId == 0) {
                //金币掉落
                int mesos = Randomizer.nextInt(1 + Math.abs(de2.Maximum+hideAttribute.totalMesoRateCount));
        ...
        if (monster.getStats().isBoss()) {
            if (chr.getParty() == null || chr.getParty().getMembers().size() == 1) {
                ...
                if ((int) Integer.valueOf(LtMS.ConfigValuesMap.get((Object) "屠令广播开关")) >= 1) {
                    Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(4, "[公告]: " + chr.getName() + " 在" + chr.getClient().getChannel() + "线[" + chr.getMap().getMapName() + "](编号:" + chr.getMapId() + ")击杀了" + monster.getName() + "(编号:" + mobid + ")."));
                }
            }else {
                StringBuilder name = new StringBuilder();
                for (MaplePartyCharacter character : chr.getParty().getMembers()) {
                    final MapleCharacter curChar = chr.getMap().getCharacterById(character.getId());
                    if (curChar != null) {
                ...
                if ((int) Integer.valueOf(LtMS.ConfigValuesMap.get((Object) "屠令广播开关")) >= 1) {
                    Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(4, "[公告]: " + name.toString() + " 在" + chr.getClient().getChannel() + "线[" + chr.getMap().getMapName() + "](编号:" + chr.getMapId() + ")击杀了" + monster.getName() + "(编号:" + mobid + ")."));
                }
            }
    ...
    public final void killMonster(final MapleMonster monster, MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, int lastSkill) {
        ...
        if (WorldBoss.contains(monster)) {
            WorldBoss.recordKill(chr.getId());
            monster.setHp(0L);
            Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[世界BOSS] " + chr.getName() + " 经过不懈努力，终于耗尽了 " + monster.getStats().getName() + " 的最后一滴血，获得了绝杀奖励， 其他参与者也可以来领取奖励了！~"));
        }
        if (!World.getOutsideBoss().isEmpty() && !World.getOutsideMap().isEmpty()) {

            ArrayList<Pair<Integer, Integer>> outsideBoss = World.getOutsideBoss();
            for (Pair<Integer, Integer> one : outsideBoss) {
                if (one.left == monster.getId() && one.right == this.mapid) {
                    for (int i = 0; i < World.bossAndMapRecord.size(); ++i) {
                        if (((MapleMonster) ((Pair) World.bossAndMapRecord.get(i)).left).getId() == mobid && ((MapleMap) ((Pair) World.bossAndMapRecord.get(i)).right).getId() == this.mapid) {
                            World.bossAndMapRecord.remove(i);
                            break;
                        }
                    }
                    if (chr.getParty() != null) {
                        ...
                        while (var42.hasNext()) {
                            MaplePartyCharacter character = (MaplePartyCharacter) var42.next();
                            MapleCharacter curChar = chr.getMap().getCharacterById(character.getId());
                            if (curChar != null) {
                                this.增加积分(curChar, (Integer) one.right);
                                curChar.dropMessage(5, "你获得了" + one.right + "积分。");
                            }
                        }
                    } else {
                        this.增加积分(chr, (Integer) one.right);
                        chr.dropMessage(5, "你获得了" + one.right + "积分。");
                    }

                    Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[系统公告] " + chr.getName() + " 在 " + chr.getClient().getChannel() + " 频道 " + this.getStreetName() + ":" + this.getMapName() + "，经过一番艰苦的战斗，终于战胜了 " + monster.getStats().getName() + "，获得了 " + one.right + " 积分。"));
                }
            }
        }
    ...
    private void 增加积分(MapleCharacter chr, int mount) {
        ...
        try (Connection con = DBConPool.getConnection()){
            ...
            //服务端输出信息.println_err("随机BOSS线程，击杀广播提醒读取数据库错误，错误原因：" + var7);
...
//        double finalTotDamageToOneMonster = totDamageToOneMonster;
        applyfrom.triggeredEquipmentsList.forEach(e ->{
            if(StringUtils.isNotEmpty(e.getSkillTx()) && e.getSkillId()==effect.getSourceId()){
                applyfrom.getClient().sendPacket(UIPacket.AranTutInstructionalBalloon(e.getSkillTx()));
            }});
        ScheduledFuture<?> poisonSchedule = tMan.register((Runnable)new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (MapleMapObject mo : applyfrom.getClient().getPlayer().getMap().getMapObjectsInRect(mist.getBox(), Collections.singletonList(MapleMapObjectType.MONSTER))) {
                            ...
                            if(index > superSkills.getSkillCount() ) {break;}
                            if ( Objects.nonNull(applyfrom.getMap()) &&  ((MapleMonster)mo).getId()!=9300061){
                                long l = (long) ( applyfrom.getStat().damage * (superSkills.getHarm()/100 )+ 1)*superSkills.getSkillCount();
                                if (LtMS.ConfigValuesMap.get("世界BOSS") == ((MapleMonster) mo).getId()) {
                                    applyfrom.getMap().broadcastMessage(MobPacket.damageMonster(((MapleMonster)mo).getObjectId(),999));
                                    ((MapleMonster)mo).damage(applyfrom, t.damage(applyfrom, 999,0.0 ), true, mist.getSourceSkill().getId());
                                }else{
                                    applyfrom.getMap().broadcastMessage(MobPacket.damageMonster(((MapleMonster)mo).getObjectId(),l));
                                    ((MapleMonster)mo).damage(applyfrom, t.damage(applyfrom, l,0.0 ), true, mist.getSourceSkill().getId());
                                }
...
//                        if (applyfrom.getCorona()>0) {
                            ...
                            mist.setPosition(applyfrom.getPosition());
                            if(applyfrom.getCorona()>0){
...
//                        mist = new MapleMist(bounds, applyfrom, effect);
            ...
            //消除技能
                ...
                tMan.schedule((Runnable)new Runnable() {
                    ...
                    public void run() {
                        if(applyfrom.getCorona()>0){
                            applyfrom.setCoronaJan(applyfrom.getCorona());
                        }
                        if (poisonSchedule2 != null) {
                            poisonSchedule2.cancel(false);
                        }
...
public void sendSkill(MapleCharacter chr,final MapleStatEffect effect){
    ...
    public void spawnMobDrop( IItem idrop, final Point dropPos, final MapleMonster mob, MapleCharacter chr, final byte droptype, final short questid) {
        ...
        if (GameConstants.getInventoryType(idrop.getItemId()) == MapleInventoryType.EQUIP && mob.getStats().isBoss()) {
            //暗黑破坏神之冒险之神
    ...
    private  boolean extracted(int idrop, MapleCharacter chr,int quantity) {
        ...
        if (zudui<=1) {
            ArrayList<Integer> sellWhenPickUpItemList = chr.getSellWhenPickUpItemList();
            if (sellWhenPickUpItemList.contains(idrop)) {
                MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                int price;
                if (!GameConstants.isThrowingStar(idrop) && !GameConstants.isBullet(idrop)) {
                    price = (int) (ii.getPrice(idrop) *  quantity);
                } else {
                    price = (int) ((float) ii.getWholePrice(idrop) / (float) ii.getSlotMax(chr.getClient(), idrop));
                }
    ...
    private void 宠物吸物(MapleCharacter chr, MapleMapItem mdrop) {
        ...
        if (chr.getId() == mdrop.character_ownerid) {
            for ( MaplePet pet : chr.getSummonedPets()) {
                if (chr.getItemQuantity(LtMS.ConfigValuesMap.get("宠吸道具"), false) > 0 && pet.getPetItemId() != 0) {
                    宠物数据库ID = pet.getUniqueId();
                    吸物状态 = true;
                    break;
                }
    ...
    /**
     * 激活地图上的物品反应器
     * 当满足特定条件时，此方法会激活地图上的反应器，通常用于特定物品掉落时触发的事件
     *
     * @param drop 地图上掉落的物品
     * @param c 触发反应器的玩家客户端
     */
...
//        RedisUtil.hset(RedisUtil.KEYNAMES.DOMAIN_AURA.getKeyName(), chr.getId()+"","0");
        ...
        if (!GameConstants.isBossMap(chr.getMap().getId()) && this.isRespawnOnePoint()) {
            boolean isExist = false;
            Iterator var29 = this.getCharactersThreadsafe().iterator();

            while(var29.hasNext()) {
                MapleCharacter chr0 = (MapleCharacter)var29.next();
                if (chr0 != null && this.getOwnerList().contains(chr0.getId())) {
                    isExist = true;
                }
            }

            if (!isExist) {
                this.setMonsterCanMove(false);
                this.clearOwnerList();
                this.setRespawnOnePoint(false);
                this.reLoadMonsterSpawn();
                this.loadMonsterRate(false);
                this.removeOwner(chr.getId());
                this.killAllMonsters(true, false);
            }
        }
    ...
    public void addAreaMonsterSpawn(final MapleMonster monster, Point pos1, Point pos2, Point pos3, int mobTime, final String msg) {
        ...
        if (monsterSpawn.stream().noneMatch(s -> s.getMonster().getId() == monster.getId() && s.getMonster().getStats().isBoss())){
...
public void respawn(final boolean force) {
    ...
    //刷新怪物的时间
        createMobInterval = (short) (LtMS.ConfigValuesMap.get("地图刷新频率") >0  ?  LtMS.ConfigValuesMap.get("地图刷新频率") : 5000); // 轮回有輪迴時怪物重生時間間隔为 0 轮回石碑
//        return lastSpawnTime > 0 && isSpawns && lastSpawnTime + createMobInterval < System.currentTimeMillis();
                ...
                try {
                    ...
                    if (kickOut && (Boolean)MapleMap.this.clock_s_thread_map.get(cThread)) {
                        ...
                        while(var2.hasNext()) {
                            ...
                            MapleMap.this.clock_s_thread_map.remove(cThread);
                            return;
                        }
                    }
    ...
    public boolean addOwner(MapleParty party) {
        if (party == null) {
            return false;
        } else {
            Iterator var2 = party.getMembers().iterator();

            while(var2.hasNext()) {
                MaplePartyCharacter member = (MaplePartyCharacter)var2.next();
                if (member != null) {
                    this.addOwner(member.getId());
                }
            }
    ...
    public boolean addOwner(MapleMap map) {
        if (map == null) {
            return false;
        } else {
            Iterator var2 = map.getCharactersThreadsafe().iterator();

            while(var2.hasNext()) {
                MapleCharacter chr = (MapleCharacter)var2.next();
                if (chr != null) {
                    this.addOwner(chr.getId());
                }
            }
    ...
    public boolean removeOwner(int chrId) {
        if (this.ownerList.contains(chrId)) {
            if (ListUtil.isNotEmpty(ownerList)) {
    ...
    public boolean removeOwner(MapleParty party) {
        if (party == null) {
            return false;
        } else {
            Iterator var2 = party.getMembers().iterator();
    ...
    public final void broadcastMessageChair(MapleCharacter source, byte[] packet, boolean repeatToSource) {
        this.broadcastMessageChair(repeatToSource ? null : source, packet, Double.POSITIVE_INFINITY, source.getPosition());
    }
    private void broadcastMessageChair(MapleCharacter source, byte[] packet, double rangeSq, Point rangedFrom) {
        this.charactersLock.readLock().lock();

        try {
            Iterator var6 = this.characters.iterator();

            while(var6.hasNext()) {
                MapleCharacter chr = (MapleCharacter)var6.next();
                if (chr != source && chr.isShowChair()) {
                    if (rangeSq < Double.POSITIVE_INFINITY) {
                        if (rangedFrom.distanceSq(chr.getPosition()) <= rangeSq) {
                            chr.getClient().sendPacket(packet);
                        }
