...
public class MapleMiniGame extends AbstractPlayerStore
{
    ...
    private int[][] piece;
    private List<Integer> matchcards;
    int loser;
    int turn;
    int piecetype;
    int firstslot;
    ...
    public int getCardId(final int slot) {
        return (int)Integer.valueOf(this.matchcards.get(slot - 1));
    }
            ...
            for (int y = 0; y < 15; ++y) {
                for (int x = 0; x < 15; ++x) {
                    if (!found && this.searchCombo(x, y, type)) {
                        this.broadcastToVisitors(PlayerShopPacket.getMiniGameResult(this, 2, (int)this.getVisitorSlot(chr)));
                        this.setOpen(true);
                        this.update();
                        this.checkExitAfterGame();
                        found = true;
