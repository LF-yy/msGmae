...
public class MapleGuildMatch extends MapleEvent {
    ...
    public MapleGuildMatch(int channel, int[] mapid) {
        super(channel, mapid);
    }
    ...
    public void finished(MapleCharacter chr) {
        if (!this.isRunning) {
            ...
            byte type;
            if (this.count <= 3) {
                this.givePrizeS(chr, MapleEventType.家族对抗赛, 1);
                type = 1;
            } else if (this.count > 3 && this.count <= 10) {
                this.givePrizeS(chr, MapleEventType.家族对抗赛, 2);
                type = 2;
            } else {
                this.givePrizeS(chr, MapleEventType.家族对抗赛, 3);
                type = 3;
            }
            ...
            while(var3.hasNext()) {
                ...
                while(var5.hasNext()) {
                    ...
                    chr0.dropMessage(6, "[家族对抗赛活动]恭喜 " + chr.getName() + " 归还了遗失的宝物，他是第 " + this.count + " 名完成的玩家，获得了 " + type + " 等奖。");
                }
            }
    ...
    public void startEvent() {
        ...
        this.startGameSchedule = EventTimer.getInstance().schedule(new Runnable() {
            public void run() {
                MapleGuildMatch.this.TimeOut();
                MapleGuildMatch.this.unreset();
            }
        }, this.time);
        this.broadcast(MaplePacketCreator.serverNotice(0, "活动已经开始，请大家抓紧时间收集参赛点。"));
    }
    ...
    public void resetSchedule() {
        ...
        if ((Integer) LtMS.ConfigValuesMap.get("家族对抗赛准备时长") > 0) {
            this.preparationTime = (long)((Integer)LtMS.ConfigValuesMap.get("家族对抗赛准备时长") * 60 * 1000);
        }

        if ((Integer)LtMS.ConfigValuesMap.get("家族对抗赛比赛时长") > 0) {
            this.time = (long)((Integer)LtMS.ConfigValuesMap.get("家族对抗赛比赛时长") * 60 * 1000);
        }
                ...
                if (this.guildPointsMap.size() == 1) {
                    ...
                    this.prepareGameSchedule = EventTimer.getInstance().schedule(new Runnable() {
                        public void run() {
                            Iterator var1;
                            MapleCharacter chr;
                            if (MapleGuildMatch.this.guildPointsMap.size() >= 2) {
                                var1 = MapleGuildMatch.this.getAllCharacters().iterator();

                                while(var1.hasNext()) {
                                    chr = (MapleCharacter)var1.next();
                                    if (chr != null) {
                                        MapleGuildMatch.this.warpBack(chr);
                                    }
                                }

                                var1 = MapleGuildMatch.this.getAllMaps().iterator();
                                ...
                                var1 = MapleGuildMatch.this.getAllJoinedChr().iterator();

                                while(true) {
                                    do {
                                        if (!var1.hasNext()) {
                                            MapleGuildMatch.this.dropEffectMessageInMaps(5120009, "比赛正式开始~大家请抓紧时间击败怪物和对手！");
                                            MapleGuildMatch.this.startEvent();
                                            return;
                                        }
                                    ...
                                    chr.changeMap(MapleGuildMatch.this.mapid[0], 0);
    ...
    public int join(MapleCharacter chr) {
        if (chr == null) {
            ...
            if (guild == null) {
                ...
                return -3;
            } else if (((ArrayList)this.guildChrMap.get(chr.getGuildId())).contains(chr.getId())) {
                return -4;
            } else if (this.isBegain()) {
                return -5;
            } else {
        ...
        if (this.guildPointsMap.containsKey(guildId)) {
            ...
            while(var4.hasNext()) {
                ...
                if (chr != null && chr.getGuildId() == guildId) {
                    if (points >= 0) {
                        chr.dropMessage(5, "[家族对抗赛]你的家族增加了 " + points + " 点数，目前总点数为 " + this.getPoints(guildId) + "。");
                    } else {
                        chr.dropMessage(5, "[家族对抗赛]你的家族减少了 " + points + " 点数，目前总点数为 " + this.getPoints(guildId) + "。");
                    }
                }
            }
    ...
    private void timeThread() {
        this.msgSchedule = WorldTimer.getInstance().register(new Runnable() {
            public void run() {
                ...
                if (MapleGuildMatch.this.getRegisterTimeLeft() > 0L) {
                    MapleMap map = MapleGuildMatch.this.getChannelServer().getMapFactory().getMap(229010000);
                    if (map != null) {
                        map.给时钟((int)MapleGuildMatch.this.getRegisterTimeLeft() / 60 / 1000, false, false);
                    }

                    if (minutes % 2 == 0) {
                        MapleGuildMatch.this.WorldMessage(2, "[家族对抗赛] : 比赛正在报名中，还有 " + MapleGuildMatch.this.getRegisterTimeLeft() / 60L / 1000L + " 分 " + seconds + " 秒开始，请抓紧时间报名，倒计时结束自动开始。");
                    }
                }

                if (MapleGuildMatch.this.getTimeLeft() > 0L && minutes % 2 == 0) {
                    MapleGuildMatch.this.WorldMessage(2, "[家族对抗赛] : 比赛正在进行中，还有 " + MapleGuildMatch.this.getTimeLeft() / 60L / 1000L + " 分 " + seconds + " 秒结束，大家加油！");
                    String text = "[家族对抗赛] : 各家族参赛点统计：";
                    int count = 1;
                    Iterator var6 = MapleGuildMatch.this.getGuildIdsOrderByPoints().entrySet().iterator();

                    while(var6.hasNext()) {
                        Map.Entry<Integer, Integer> entry = (Map.Entry)var6.next();
                    ...
                    MapleGuildMatch.this.WorldMessage(2, text);
                }
    ...
    private void distributeRewards() {
        ...
        int count = 0;
        Iterator var11 = this.getAllJoinedChr().iterator();

        while(var11.hasNext()) {
            MapleCharacter chr = (MapleCharacter)var11.next();
            if (chr != null) {
                this.givePrizeS(chr, MapleEventType.家族对抗赛, (Integer)guildRewardsTypeMap.get(chr.getGuildId()));
                ++count;
            }
        }
        ...
        while(var6.hasNext()) {
            ...
            MapleGuild guild = Guild.getGuild((Integer)entry.getKey());
            if (guild != null) {
                int gp0 = (Integer)entry.getValue() / rate;
                if (gp0 > 0) {
        ...
        while(var1.hasNext()) {
            ...
            while(var3.hasNext()) {
                ...
                if (map != null) {
                    ...
                    map.startMapEffect("[家族对抗赛]比赛结束，<" + Guild.getGuild(this.championGuildId).getName() + ">家族以" + this.guildPointsMap.get(this.championGuildId) + "参赛点赢得了比赛，让我们恭喜他们！", 5120009);
                }
            }
        }
