...
public class MapleSnowball extends MapleEvent
{
    ...
    public static class MapleSnowballs
    {
        ...
        public static void hitSnowball(MapleCharacter chr) {
            ...
            final MapleSnowball sb = (MapleSnowball)chr.getClient().getChannelServer().getEvent(MapleEventType.推雪球比赛);
                ...
                if (!snowman) {
                    ...
                    if (damage > 0) {
                        ...
                        if (ball.getSnowmanHP() <= 0) {
                            ...
                            final MapleSnowballs oBall = sb.getSnowBall((int)((team == 0) ? 1 : 0));
                            oBall.setHittable(false);
                            final MapleMap map2 = chr.getMap();
                            oBall.broadcast(map2, 4);
                            oBall.snowmanSchedule = EventTimer.getInstance().schedule((Runnable)new Runnable() {
                                @Override
                                public void run() {
