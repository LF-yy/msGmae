...
public class MapleFamilyCharacter implements Serializable
{
    ...
    public boolean equals(final Object other) {
        ...
        return o.getId() == this.id && o.getName().equals((Object)this.name);
    }
    ...
    public List<MapleFamilyCharacter> getAllJuniors(final MapleFamily fam) {
        ...
        if (this.junior1 > 0) {
            ...
            if (chr != null) {
                ret.addAll((Collection<? extends MapleFamilyCharacter>)chr.getAllJuniors(fam));
            }
        }
        if (this.junior2 > 0) {
    ...
    public void resetPedigree(final MapleFamily fam) {
        (this.pedigree = new ArrayList<Integer>()).add(Integer.valueOf(this.id));
        if (this.seniorid > 0) {
    ...
    public int resetGenerations(final MapleFamily fam) {
        int descendants1 = 0;
        int descendants2 = 0;
        if (this.junior1 > 0) {
            final MapleFamilyCharacter chr = fam.getMFC(this.junior1);
        ...
        final int ret = Math.max(descendants1, descendants2);
        final int n= ret;
        return n + ((n  > 0) ? 1 : 0);
    }
