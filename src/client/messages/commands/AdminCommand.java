...
public class AdminCommand
{
    ...
    public static class 给道具 extends CommandExecute
    {
        @Override
        public boolean execute(final MapleClient c, final String[] splitted) {
            if (splitted.length < 3) {
            ...
            final String name = splitted[1];
            final int gain = Integer.parseInt(splitted[2]);
            final int sl = Integer.parseInt(splitted[3]);
            final int ch = Find.findChannel(name);
            if (ch <= 0) {
                c.getPlayer().dropMessage(6, "玩家必须上线");
                return true;
            ...
            {
                victim.gainItem(gain, sl);
                final String msg = "[GM 密语] GM " + c.getPlayer().getName() + " 给了 " + victim.getName()  + " " + gain + "道具";
                Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, msg));
            }
            return true;
        }
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!给道具 <名字> <物品ID> <数量> - 给玩家道具").toString();
        }
    }
    ...
    public static class CloneMe extends CommandExecute
    {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!cloneme - 开启身外之身").toString();
        }
    }
    ...
    public static class 刷 extends CommandExecute
    {
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            if (GameConstants.isPet(itemId)) {
                ...
                item.setGMLog(c.getPlayer().getName());
                MapleInventoryManipulator.addbyItem(c, item);
                if ((int)Integer.valueOf(LtMS.ConfigValuesMap.get((Object)"指令通知开关")) <= 0) {
