...
public class 鱼来鱼往 extends javax.swing.JFrame {
    ...
    public void 按键开关(String a, int b) {
        ...
        if (检测开关 > 0) {
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
                ps1 = con.prepareStatement("SELECT * FROM configvalues WHERE name = ?");
                ps1.setString(1, a);
                rs = ps1.executeQuery();
                if (rs.next()) {
        ...
        if (result1 && result2 && result3) {
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
                ...
                if (rs.next()) {
                    ...
                    sqlString1 = "update configvalues set Val = '" + 飞鱼数量.getText() + "' where name = '飞鱼数量';";
                    PreparedStatement Val = con.prepareStatement(sqlString1);
                    Val.executeUpdate(sqlString1);

                    String sqlString2 = null;
                    sqlString2 = "update configvalues set Val = '" + 金币倍率.getText() + "' where name = '飞鱼金币倍率';";
                    PreparedStatement Val3 = con.prepareStatement(sqlString2);
                    Val3.executeUpdate(sqlString2);

                    String sqlString3 = null;
                    sqlString3 = "update configvalues set Val = '" + 点券倍率.getText() + "' where name = '飞鱼点券倍率';";
